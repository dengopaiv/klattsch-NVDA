...
```
  ...
  Phase 2   Rewrite in C      -> REWRITE.md steps 0-6     [not started]
  Phase 3   Extend the engine -> REWRITE.md step 7
...
[COMPARISON.md](COMPARISON.md), against the twelve synthesizers surveyed in
`..\speech synths overview.md`. Four things came out of it:
...
[GENERATOR.md](GENERATOR.md). Native Win32, one executable per architecture,
linking the same static engine the add-on does.
...
[NVDA-ADDON.md](NVDA-ADDON.md). One Python shim, one native library per
